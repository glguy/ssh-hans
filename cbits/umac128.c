...
static
void nh_transform(nh_ctx *hc, const UINT8 *buf, UINT32 nbytes)
...
static void umac_verify(void)
{
    ...
    ctx = umac128_new("abcdefghijklmnop");
    ...
    for (i = 1; i < (int)(sizeof(inc)/sizeof(inc[0])); i++) {
            for (j = 0; j <= data_len-inc[i]; j+=inc[i])
                umac128_update(ctx, data_ptr+j, inc[i]);
            umac128_final(ctx, tag, nonce);
            umac128_reset(ctx);
                           ...
                           int data_len, double hz)
{
    ...
    if (nbytes <= data_len) {
        ...
        umac128_update(ctx, data_ptr, data_len);
        umac128_final(ctx, tag, nonce);
        ticks = clock();
        do {
            j = iters_per_tag;
            ...
            if (remaining)
                umac128_update(ctx, data_ptr, remaining);
            umac128_final(ctx, tag, nonce);
            nonce[7] += 1;
        } while (--i);
        ticks = clock() - ticks;
...
static void speed_test(void)
{
    ...
    for (i = 0; i < data_len; i++)
        data_ptr[i] = (i*i) % 128;
    ctx = umac128_new("abcdefghijklmnopqrstuvwxyz");
